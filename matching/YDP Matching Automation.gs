...
function getYdpMatchingDataDictionaryRows_() {
  return [
    ...
    ['Sheet', YDP_MATCHING_CONFIG.sheets.matchedPairs, 'Match Status', 'HOW the pairing was formed and where it sits in the matching workflow. Auto-match stamps every new pair "Auto-Matched" (chosen by the algorithm from Pair Scores, not a human). Expected values: Auto-Matched (system default); Manually Adjusted (you swapped the mentor); Confirmed / Finalized (you have signed off). The system only ever writes Auto-Matched; the rest are labels you apply.', 'Update as you review pairings; leave as Auto-Matched until reviewed.'],
    ['Sheet', YDP_MATCHING_CONFIG.sheets.matchedPairs, 'Active Status', 'WHERE the pair is in the actual mentorship lifecycle. Auto-match stamps every new pair "Pending Review" (created but not yet approved or started). Expected values: Pending Review (system default); Active (mentorship underway); Paused or Inactive (temporarily on hold); Ended or Completed (finished). The system only ever writes Pending Review; you set the rest as the cohort runs.', 'Update as each mentorship starts, pauses, or ends.'],
    ['Sheet', YDP_MATCHING_CONFIG.sheets.matchedPairs, 'Risk Status', 'A manual flag for pairs that need attention. Expected values: blank or Not Started (nothing to flag yet); On Track; At Risk (inactive, no-shows, or a reported problem). The system does not set this; it is for your monitoring notes.', 'Used later to flag inactive/no-show/problem pairs during the program.'],
