...
Important: `Setup matching workbook` does not import mentor or mentee rows, but it does more than create tabs. It rewrites the four supported `Source Config` rows while preserving their current known values, and it can convert blank or old `Pending Review` statuses from saved final scores into the current review wording. The snapshot tabs stay blank until `Sync source snapshots from forms` runs successfully.
...
| Button | What it does in plain English | When to use it | Be careful |
| --- | --- | --- | --- |
| `Setup matching workbook` | Creates/repairs matching tabs, rewrites the four supported configuration rows, and normalizes old review-status wording from saved final scores. | Use once at the start, or rerun if tabs are missing. | It does not import source rows or send emails. Do not store unsupported custom rows in `Source Config`, because setup removes them. |
| `Sync source snapshots from forms` | Runs setup, then copies the latest live mentee and mentor form rows into this workbook. | Use before scoring or matching, especially after new people register. | It replaces both snapshot tabs, rewrites standard config rows, and may normalize old review statuses. Completed Mentee Scores and Pair Scores remain. |
...
| `Auto-match from pair scores` | Uses saved pair scores to select the best available mentor for each fully scored mentee. | Use only after all available mentors are scored for each eligible mentee and before live match emails are sent. | It does not call Gemini. It clears and rebuilds `Match Recommendations` and `Matched Pairs`, including statuses, dates, notes, and email tracking. Export/backup those tabs first if any existing history must be preserved. |
